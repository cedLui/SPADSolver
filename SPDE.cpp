...
std::vector<double> AbsorptionProbDist(double Width, int Steps, std::vector<double> AlFracProfile, double PhotonEnergy){
    ...
    double prevGamma = gamma(AlFracProfile[0], PhotonEnergy);
    double Gamma;
    double IntGamma;
...
std::vector<double> AvaPairProb(double Width, int Steps, double Accuracy, double EField){
    ...
    std::vector<std::vector<double>> PVec = avaProb(Width, Steps, Accuracy, EField); //Calculate P_e and P_h
...
double SPDE(double RefInd, double Width, int Steps, std::vector<double> AlFracProfile, double PhotonEnergy, double Accuracy, double EField){
    ...
    std::vector<double> avaPairProb = AvaPairProb(Width, Steps, Accuracy, EField); //Calculate probability of an avalanche at points through the device
    ...
    for (int i = 1; i < Steps; i++){ //Calculate IntAbsAva using a trapazoidal sum 
